    ...
    <div class="mt-10 sm:mt-0">
        <div class="md:grid md:grid-cols-3 md:gap-6">
          ...
          <div class="mt-5 md:mt-0 md:col-span-2">
            <form action="/books/{{ $bookFind->id }}" method="POST" enctype="multipart/form-data">
              @method('put')
                @csrf
                    ...
                    <div class="col-span-6 sm:col-span-6 lg:col-span-2">
                      <label for="image" class="block text-sm font-medium text-gray-700">Image</label>
                        @error('image')
                            <span class="mt-2 text-center text-sm text-red-600">{{ $message }}</span>
                        @enderror
                      <input required type="file" name="image" id="image" autocomplete="address-level2" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full h-10 px-2 shadow-md sm:text-sm border-gray-300 rounded-md" value="{{ old('image', $bookFind->image) }}">
                    </div>
