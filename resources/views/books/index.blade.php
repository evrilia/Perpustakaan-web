        ...
        <form action="/books" method="GET">
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                <div class="input-group">
                    <input type="text" class="form-control" name="search" placeholder="Search books data" aria-label="Search books data" aria-describedby="button-addon2">
            ...
            <table class="table-auto w-full">
              <thead class="bg-gray-50 border-b-2 border-gray-200">
                <tr>
                  ...
                  <th scope="w-20 p-3 text-sm font-semibold tracking-wide text-left">Image</th>
              ...
              <tbody class="divide-y divide-gray-200">
                @foreach ($books as $book)    
                  <tr>
                    ...
                    {{-- images --}}
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <img src="{{ asset('/storage/public/' . $book->image) }}" alt="{{$book->title}}" width="120px" hight="120px"  class="img-thumbnail">
                      {{-- <img src="{{ asset('storage/' . $book->image) }}" alt="{{$book->title}}" width="120px" hight="120px"  class="img-thumbnail"> )}}" --}}
                    </td>
                    
                    <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      <a href="{{ url('/books/' . $book->id . '/edit') }}" type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-200 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 no-underline">Edit</a>
            ...
            <div class="bg-white space-y-3 p-4 rounded-lg shadow">
              <div class="flex flex-col items-center space-x-2 text-sm">
                ...
                <div class="flex justify-center">
                  <img src="{{ asset('storage/' . $book->image) }}" alt="" class="w-20 h-20">
              </div>
              <div class="flex justify-center">
                <div>
                  <a href="{{ url('/books/' . $book->id . '/edit') }}" type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-200 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 no-underline">Edit</a>
                  <form action="/books/destroy" method="post" class="inline">
                    @method('delete')
                    @csrf
                    <input type="hidden" name="id" value="{{ $book->id }}">
                    <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-200 hover:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 no-underline" id="deletePost" onclick="return confirm('Are you sure you want to delete this?');">Delete</button>
                  </form>
                </div>
              </div>
            </div>
