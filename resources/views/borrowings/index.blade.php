...
<main class="bg-white shadow">
    <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-bold text-gray-900 text-center uppercase">Book borrowing</h1>
    </div>
            ...
            <div class="md:col-span-1">
              <div class="px-4 sm:px-0">
                <div class="ml-4 divide-y divide-gray-200">
                  <h3 class="text-lg font-medium leading-6 text-gray-900">Book borrowing information</h3>
                  <p class="mt-1 text-sm text-gray-600">Contains the personal data of the book borrower</p>
                </div>
                
                <div class="ml-4 px-4 py-3 bg-gray-50 text-center sm:px-6">
                  <a href="/borrowings/create" type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 no-underline">Input Borrowing</a>
                </div>
              </div>
            </div>
              ...
              <table class="table-auto w-full">
                <thead class="bg-gray-50 border-b-2 border-gray-200">
                  <tr>
                    ...
                    <th class="w-24 p-3 text-sm font-semibold tracking-wide text-left">Student's name</th>
                    <th class="w-24 p-3 text-sm font-semibold tracking-wide text-left">Student's nis</th>
                    <th class="w-20 p-3 text-sm font-semibold tracking-wide text-left">Book title</th>
                    ...
                    <th class="w-20 p-3 text-sm font-semibold tracking-wide text-left">Admin</th>
                ...
                <tbody class="divide-y divide-gray-200">
                  @foreach ($borrowings as $borrowing)    
                    <tr>
                      ...
                      <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                          {{ $borrowing->students->name}}
                      </td>
                      <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                          {{ $borrowing->students->nis}}
                      </td>
                      <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                      ...
                      <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                        @if ($borrowing->status == 'borrowed')
                          <span class="p-1.5 text-xs font-medium uppercase tracking-wider text-blue-800 bg-blue-200 rounded-lg bg-opacity-60">{{ $borrowing->status }}</span>
                        @else
                      ...
                      <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                        {{ $borrowing->admins->name }}
                      </td>
                      <td class="p-3 text-sm text-gray-700 whitespace-nowrap">
                        <a href="{{ url('/borrowings/' . $borrowing->id . '/edit') }}" type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-200 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 no-underline">Edit</a>
                        {{-- <form action="/borrowings/destroy" method="post" class="inline">
                          @method('delete')
                          @csrf
            ...
            <div class="grid grid-cols-1 xl:grid-col-2 gap-4 lg:hidden">
              ...
              <div class="bg-white space-y-3 p-4 rounded-lg shadow">
                <div class="flex flex-col items-center space-x-2 text-sm">
                  ...
                  <div class="text-center">{{  $borrowing->students->name }}</div>
                  <div class="text-center">{{ $borrowing->books->title }}</div>
                  <div>{{ $borrowing->created_at }}</div>
                  <div>{{ $borrowing->admins->name }}</div>
                  <div class="my-4"><span class="p-1.5 text-xs font-medium uppercase tracking-wider text-blue-800 bg-blue-200 rounded-lg bg-opacity-60">{{ $borrowing->status }}</span></div>
                </div>
